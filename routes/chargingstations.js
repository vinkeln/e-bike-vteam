...
router.get("/", async (req, res) => {
  try {
    ...
    const chargingStations =
      await chargingStationsModules.getchargingStations();
...
router.get("/:cityId", async (req, res) => {
  ...
  try {
    ...
    // Hämtar alla laddningsstationer från databasen via modulen
    const chargingStations =
      await chargingStationsModules.getchargingStationsBycity(cityId);
    // Returnerar resultatet som JSON med statuskod 200
    res.status(200).json({
      status: "success",
      chargingStations: chargingStations,
...
router.post("/add", checkAuth, checkAdmin, async (req, res) => {
  try {
    ...
    const existingChargingStations =
      await chargingStationsModules.checkChargingStation(latitude, longitude);
...
router.delete("/:locationId", checkAuth, checkAdmin, async (req, res) => {
  ...
  try {
    ...
    const existingChargingStations =
      await chargingStationsModules.getChargingStationLocation(locationId);
...
router.put("/update", checkAuth, checkAdmin, async (req, res) => {
  ...
  try {
    ...
    await chargingStationsModules.updateChargingStation(
      locationId,
      latitude,
      longitude,
      totalPorts
...
router.put("/update/port", checkAuth, checkAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Charging station port updated successfully" });
  } catch (error) {

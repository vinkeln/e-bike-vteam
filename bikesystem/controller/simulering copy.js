...
class BikeSimulator {
  constructor(bikeSerialNumber, initialLongitude, initialLatitude) {
    ...
    this.chargingDuration = 60 * 60 * 1000; // 1 hour
  ...
  initializeSocketEvents() {
    ...
    this.socket.on("disconnect", () => {
      ...
      this.clearIntervals(); // Anropa den nya metoden för att städa upp
    });
  ...
  simulateBatteryConsumption() {
    const reductionPerSecond = 5 / 3600; // Minskar batterinivån med 5% per timme
  ...
  stopRide() {
    console.log(`Stop ride received for bike ${this.bikeSerialNumber}`);

    // Hämta nuvarande tid
    const endTime = new Date().toISOString();

    // Hitta reseloggen som pågår
    const activeTravel = this.travelLogs.find(
      (travel) => travel.status === "in-progress"
    );

    if (activeTravel) {
      // Uppdatera reseloggen med slutdata
      activeTravel.end = {
        latitude: this.currentLatitude,
        longitude: this.currentLongitude,
        time: endTime,
      };
      activeTravel.status = "completed"; // Markera som avslutad
    }

    // Skicka uppdaterad status till servern
    this.sendStatusUpdate("ledig");

    // Stoppa gamla intervaller och starta nytt intervall för "ledig"-status
    this.stopStatusUpdates();
    this.startStatusUpdates("ledig");
  }

  // Ny metod för att stoppa alla intervall
  clearIntervals() {
    // Stoppa alla aktiva intervall
    this.stopBatteryConsumption();
    this.stopStatusUpdates();
    clearInterval(this.chargingInterval);
...
function startSimulation(bikeCount) {
  ...
  for (let i = 0; i < bikeCount; i++) {
    bikes.push(new BikeSimulator(i, 18.0686, 59.3293));
  }
  console.log(bikes);
}

...
async function updateStatus(bikeSerialNumber, locationId, status, speed, longitude, latitude, batteryLevel) {
    const query = "UPDATE scooter SET current_location_id = ?, status = ?, speed = ?, current_longitude = ?, current_latitude = ?, battery_level = ? WHERE bike_serial_number = ?";
    return await queryDatabase(query, [locationId, status, speed, longitude, latitude, batteryLevel, bikeSerialNumber]);
}

...
function initializeSocketHandlers(server) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("updateStatus", async (data) => {
            const { bikeSerialNumber, locationId, status, speed, longitude, latitude, batteryLevel} = data;
            try {
                const result = await bikeController.updateStatus(bikeSerialNumber, locationId, status, speed, longitude, latitude, batteryLevel);
